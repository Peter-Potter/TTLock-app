...
// Pure JS MD5 function for TTLock password hashing
function md5(string: string): string {
  ...
  function wordToHex(lValue: number) {
    let WordToHexValue = '', WordToHexValueTemp = '', lCount;
  ...
  try {
    ...
    if (action === 'loginWithTTLock' || action === 'linkAccount') {
      ...
      if (!username || !password) {
        return new Response(JSON.stringify({ error: 'TTLock username and password are required' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }

      // Hash password to 32-character lowercase MD5 as required by TTLock
    ...
    // ================= ACTION 2: GENERATE PASSCODE =================
    if (action === 'getPasscode') {
      const { lockId, keyboardPwdName, keyboardPwdType, startDate, endDate } = payload

      // Fetch the user's stored TTLock access token from database
      ...
      if (tokenError || !tokenData) {
        return new Response(JSON.stringify({ error: 'TTLock token not found for user. Please link your TTLock account first.' }), {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        })
      }
